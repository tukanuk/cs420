...
public class AddressBookDatabaseHelper extends SQLiteOpenHelper {
   ...
   public void addUser (com.ouda.addressbook.User user)
   {
      SQLiteDatabase db = this.getWritableDatabase();
      ContentValues values = new ContentValues();
      values.put(DatabaseDescription.User.COLUMN_USERNAME, user.userName);
      values.put(DatabaseDescription.User.COLUMN_USERPASSWORD, user.userPassword);
      long idValue = db.insert (DatabaseDescription.User.TABLE_NAME, null, values);
   }

   public com.ouda.addressbook.User Authenticate(com.ouda.addressbook.User user)
   {
      ...
      {
         com.ouda.addressbook.User user1 = new com.ouda.addressbook.User(cursor.getString(0), cursor.getString(1));
         if (user.userPassword.equals(user1.userPassword))
            return user1;
      }
      return null;
   }
